<template>
  <div class="app-container">
    ...
    >
      <el-table-column v-for="col in columns" :key="col.key" :label="col.label" :prop="col.key" :width="col.key === 'wineNameList' ? 200 : /time/ig.test(col.key) ? 150 : col.key === 'index' ? 50 : 'auto'" :fixed="col.fixed">
    ...
    <el-dialog v-if="dialogVisible" :visible.sync="dialogVisible" width="800px" title="详细信息" :close-on-click-modal="false" :close-on-press-escape="false" top="20px">
      <el-form ref="shopForm" :model="wineInfo" label-width="110px" label-position="left" hide-required-asterisk>
        <el-form-item label="渠道名称：" prop="channelName">
          <el-input v-model="wineInfo.channelName" />
        </el-form-item>
        <el-form-item label="备注：" prop="remark">
          <el-input v-model="wineInfo.remark" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList(obj = undefined) {
      ...
      getChannelListByPage(param).then(response => {
    ...
    handleFilter() {
      const param = {
        ...
        'channelName': this.listQuery.channelName || undefined,
        'startIndex': this.listQuery.page,
    ...
    confirmSend() {
      if (this.wineInfo.channelName.trim()) {
        this.updateSend = true
        const func = this.wineInfo.id ? updateProduct : insertChannel
        func({
          ...this.wineInfo,
    ...
    showRow(row) {
      this.wineInfo = {
        ...
        imageList: row.bgImage ? [{
          url: `https://www.xiaohuaxinyong.cn/productImage/${row.bgImage}`,
    ...
    async exportPhoneRow(row) {
      ...
      try {
        ...
        const response = await request({
          url: 'https://www.xiaohuaxinyong.cn/services/exportDownload',
          method: 'post',
          responseType: 'blob',
          data: params
        })
